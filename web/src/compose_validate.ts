...
export let validate = (scheduling_message: boolean, show_banner = true): boolean => {
    ...
    if (
        ...
        !(
            compose_state.message_content().trim().length >=
            topic_resolution_state.MIN_RESOLUTION_MESSAGE_LENGTH
        )
    ) {
        const message_content = compose_state.message_content();
        const should_enforce =
            topic_resolution_state.is_message_required() ||
            (topic_resolution_state.is_message_optional() && message_content.trim().length > 0);
